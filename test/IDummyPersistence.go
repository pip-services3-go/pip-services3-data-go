...
type IDummyPersistence interface {
	...
	Create(correlationId string, item Dummy) (retItem Dummy, err error)
	Update(correlationId string, item Dummy) (retItem Dummy, err error)
	...
	DeleteByIds(correlationId string, id []string) (err error)
}
